<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#kode_rtrw").focusout(function() {
			var kode_rtrw = $('#kode_rtrw').val();
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/cek_kode',
				method: "POST",
				dataType: "json",
				data: {
					'kode_rtrw': kode_rtrw
				...
				success: function(data) {
					...
					if (data.status == "exist") {
						$.alert.open('Master rtrw', 'Kode rtrw <b>"' + kode_rtrw + '"</b> sudah ada. Silahkan cek <b>View Tab</b>.', {
							OK: "OK"
						}, function(button) {
							if (button == 'OK') {
								setTimeout(function() {
						...
						$.alert.open('Master rtrw', 'Kode rtrw <b>"' + kode_rtrw + '"</b> sudah ada, tetapi <b>tidak aktif</b>.', {
							OK: "OK"
						}, function(button) {
							if (button == 'OK') {
								setTimeout(function() {
				...
				error: function() {
					$.alert.open('Master rtrw', '<center>Koneksi eror !</center>', {
						OK: "OK",
					}, function(button) {
						if (button == 'OK') {
							location.reload();
		...
		$("#save").click(function() {
			...
			} else if ($("#select_pd_desa").val() == '') {
				setTimeout(function() {
					$("#select_pd_desa").focus();
				}, 1);
			} else if ($("#nama_rtrw").val() == '') {
				setTimeout(function() {
					$("#nama_rtrw").focus();
				}, 1);
			} else if ($("#kode_rtrw").val() == '') {
				setTimeout(function() {
					$("#kode_rtrw").focus();
				}, 1);
			}
			...
			if ($("#form1").valid()) {
				$.ajax({
					url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/entry_data',
					method: "POST",
					dataType: "json",
					data: data,
					cache: false,
					...
					success: function(data) {
						if (data == "success") {
							...
							$.alert.open('Master rtrw', '<center>Data berhasil disimpan. </center>', {
								OK: "OK"
							}, function(button) {
								if (button == 'OK') {
									$("#form1")[0].reset();
					...
					error: function() {
						$.alert.open('Master rtrw', '<center>Data gagal disimpan.</center>', {
							OK: "OK",
						}, function(button) {
							if (button == 'OK') {
								location.reload();
			...
			// else{
			// 	$.alert.open('Master rtrw','Please complete the form.');
			// }
		})
		...
		$('#select_pd_provinsi').select2({
			...
			width: "100%",
			ajax: {
				url: '<?php echo base_url("index.php/rtrw/mst_rtrw/find_pd_provinsi") ?>',
				dataType: 'json',
			...
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_provinsi_detail',
				method: "POST",
				dataType: "json",
				data: {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_kabupaten_by_prov_data',
				method: "POST",
				dataType: "json",
				data: {
					'select_pd_provinsi': select_pd_provinsi
		...
		$('#select_pd_kabupaten').select2({
			...
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_kabupaten_detail',
				method: "POST",
				dataType: "json",
				data: {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_kecamatan_by_kab_data',
				method: "POST",
				dataType: "json",
				data: {
					'select_pd_kabupaten': select_pd_kabupaten
				...
				success: function(data) {
					...
					$('#select_pd_kecamatan').val(data.results[0].id).trigger('change');
				}
			});
		}).on('select2:clearing', function(e) {
		...
		$('#select_pd_kecamatan').select2({
			...
			$.ajax({
				...
				success: function(data) {
					$("#select_pd_desa").val(data[0].kddes).focusout();
				}
			});
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_desa_by_kec_data',
				method: "POST",
				dataType: "json",
				data: {
					'select_pd_kecamatan': select_pd_kecamatan
		...
		$('#select_pd_desa').select2({
			...
			var select_pd_desa = $("#select_pd_desa").val();
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/find_pd_desa_detail',
				method: "POST",
				dataType: "json",
				data: {
					'select_pd_desa': select_pd_desa
				}
			});
		}).on('select2:clearing', function(e) {
			$('#select_pd_desa').val(null).trigger('change');
		});
		...
		$("#search_kode_rtrw").keyup(function() {
			setTimeout(function() {
				var text = $("#search_kode_rtrw").val();
				var postdata = $("#grid_tab_view").jqGrid('getGridParam', 'postData');
				$.extend(postdata, {
					filters: 'kode_rtrw',
					searchField: 'kode_rtrw',
					searchOper: 'bw',
					search_kode_rtrw: text,
				});
		...
		$("#grid_tab_view").jqGrid({
			url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/grid_view',
			datatype: "json",
			height: 'auto',
			width: 800,
			shrinkToFit: false,
			...
			editurl: "<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/edit",
			rowNum: 10,
			rowList: [10, 20, 30, 100],
			caption: "Display Master rtrw",
			viewrecords: true,
			sortname: 'rt',
			sortorder: "asc",
		...
		$("#delete").click(function() {
			...
			var kode_rtrw = $('#grid_tab_view').getCell(gr, 'rt');
			if (gr != null) {
				$.alert.open('Master rtrw', '<center>Hapus Kode rtrw <b>"' + kode_rtrw + '"</b> ?</center>', {
					OK: "OK",
					NO: "NO"
				}, function(button) {
					if (button == 'OK') {
						$.ajax({
							url: '<?php echo base_url(); ?>index.php/rtrw/mst_rtrw/edit',
							method: 'POST',
							dataType: "json",
							data: {
								'rt': kode_rtrw,
							...
							success: function(data) {
								...
								if (data == "success") {
									$('#grid_tab_view').trigger('reloadGrid');
									$.alert.open('Master rtrw', '<center>Data berhasil dihapus.</center>', {
										OK: "OK"
									}, function(button) {
										if (button == 'OK') {} else {}
									});
							...
							error: function() {
								$.alert.open('Master rtrw', '<center>Data gagal dihapus.</center>', {
									OK: "OK"
								}, function(button) {
									if (button == 'OK') {
										location.reload();
				...
				$.alert.open('Master rtrw', '<center>Pilih baris yang akan dihapus.</center>', {
					OK: "OK",
				}, function(button) {
		...
		<div class="row">
			<div class="twelve columns">
				<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix">
					<h6>&nbsp;<img src="<?php echo base_url(); ?>images/crud/home.png">&nbsp;Master rtrw</h6>
				</div>
				<hr>
					...
					<section>
						...
						<div class="content" data-section-content>
							<form id="form1" action="" method="post">
								...
								<div class="row">
									<div class="large-3 columns">
										<label class="right inline"><span class="radius secondary label">Kode dan Nama Desa<span style="color: red;font-size: 22px;">*</span> :</label>
									</div>
									<div class="large-4 columns">
										<select name="select_pd_desa" id="select_pd_desa" tabindex="3"></select>
									</div>
									<div class="large-1 columns">
									</div>
								</div>
								<div class="row">
									<div class="large-3 columns">
										<label class="right inline"><span class="radius secondary label">RT <span style="color: red;font-size: 22px;">*</span> :</span></label>
									</div>
									<div class="large-2 columns">
										<input type="text" name="kode_rtrw" id="kode_rtrw" onBlur="javascript:{this.value = this.value.toUpperCase(); }" tabindex="1" />
									</div>
									<div class="large-1 columns">
									</div>
								</div>
								<div class="row">
									<div class="large-3 columns">
										<label class="right inline"><span class="radius secondary label">RW <span style="color: red;font-size: 22px;">*</span> :</span></label>
									</div>
									<div class="large-4 columns">
										<input type="text" name="nama_rtrw" id="nama_rtrw" tabindex="2" />
									</div>
									<div class="large-1 columns">
									</div>
								</div>
							...
							<form id="form2">
								<div class="row">
									<div class="large-12 columns">
										<div class="large-4 columns">
											<label class="right inline"><span class="radius secondary label">Search Kode rtrw atau Nama : </label>
										</div>
										<div class="large-3 columns">
											<input type="text" id="search_kode_rtrw" class='iblue' name="search_kode_rtrw" tabindex="1" />
										</div>
										<hr>
