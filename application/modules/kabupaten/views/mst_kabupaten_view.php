<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#kode_kabupaten").focusout(function() {
			var kode_kabupaten = $('#kode_kabupaten').val();
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/cek_kode',
				method: "POST",
				dataType: "json",
				data: {
				...
				success: function(data) {
					...
					if (data.status == "exist") {
						$.alert.open('Master kabupaten', 'Kode kabupaten <b>"' + kode_kabupaten + '"</b> sudah ada. Silahkan cek <b>View Tab</b>.', {
							OK: "OK"
						}, function(button) {
							if (button == 'OK') {
								setTimeout(function() {
									$("#kode_kabupaten").val("").focus();
								}, 1);
							} else {}
						});
					} else if (data.status == "inactive") {
						$.alert.open('Master kabupaten', 'Kode kabupaten <b>"' + kode_kabupaten + '"</b> sudah ada, tetapi <b>tidak aktif</b>.', {
							OK: "OK"
						}, function(button) {
							if (button == 'OK') {
								setTimeout(function() {
				...
				error: function() {
					$.alert.open('Master kabupaten', '<center>Koneksi eror !</center>', {
						OK: "OK",
					}, function(button) {
						if (button == 'OK') {
							location.reload();
		...
		$("#save").click(function() {
			...
			if ($("#select_pd_provinsi").val() == '') {
				...
				setTimeout(function() {
					$("#nama_kabupaten").focus();
				}, 1);
			} else if ($("#kode_kabupaten").val() == '') {
				setTimeout(function() {
					$("#kode_kabupaten").focus();
				}, 1);
			}
			...
			if ($("#form1").valid()) {
				$.ajax({
					url: '<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/entry_data',
					method: "POST",
					dataType: "json",
					data: data,
					cache: false,
					...
					success: function(data) {
						if (data == "success") {
							...
							$.alert.open('Master kabupaten', '<center>Data berhasil disimpan. </center>', {
								OK: "OK"
							}, function(button) {
								if (button == 'OK') {
									$("#form1")[0].reset();
					...
					error: function() {
						$.alert.open('Master kabupaten', '<center>Data gagal disimpan.</center>', {
							OK: "OK",
						}, function(button) {
							if (button == 'OK') {
								location.reload();
			...
			// else{
			// 	$.alert.open('Master kabupaten','Please complete the form.');
			// }
		})
		...
		$('#select_pd_provinsi').select2({
			...
			width: "100%",
			ajax: {
				url: '<?php echo base_url("index.php/kabupaten/mst_kabupaten/find_pd_provinsi") ?>',
				dataType: 'json',
			...
			$.ajax({
				url: '<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/find_pd_provinsi_detail',
				method: "POST",
				dataType: "json",
				data: {
				...
				success: function(data) {
					$("#provinsi_read").val(data[0].trayek_pp).focusout();
					$("#provinsi_read").val(data[0].rute_berangkat).focusout();
					$("#provinsi_kembali_read").val(data[0].rute_kembali).focusout();
				}
			});
		...
		$("#search_kode_kabupaten").keyup(function() {
			setTimeout(function() {
				var text = $("#search_kode_kabupaten").val();
				var postdata = $("#grid_tab_view").jqGrid('getGridParam', 'postData');
				$.extend(postdata, {
					filters: 'kode_kabupaten',
					searchField: 'kode_kabupaten',
					searchOper: 'bw',
					search_kode_kabupaten: text,
				});
		...
		$("#grid_tab_view").jqGrid({
			url: '<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/grid_view',
			datatype: "json",
			height: 'auto',
			width: 800,
			shrinkToFit: false,
			...
			editurl: "<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/edit",
			rowNum: 10,
			rowList: [10, 20, 30, 100],
			caption: "Display Master kabupaten",
			viewrecords: true,
			sortname: 'kdkab',
			sortorder: "asc",
		...
		$("#delete").click(function() {
			...
			var kode_kabupaten = $('#grid_tab_view').getCell(gr, 'kdkab');
			if (gr != null) {
				$.alert.open('Master kabupaten', '<center>Hapus Kode kabupaten <b>"' + kode_kabupaten + '"</b> ?</center>', {
					OK: "OK",
					NO: "NO"
				}, function(button) {
					if (button == 'OK') {
						$.ajax({
							url: '<?php echo base_url(); ?>index.php/kabupaten/mst_kabupaten/edit',
							method: 'POST',
							dataType: "json",
							data: {
							...
							success: function(data) {
								...
								if (data == "success") {
									$('#grid_tab_view').trigger('reloadGrid');
									$.alert.open('Master kabupaten', '<center>Data berhasil dihapus.</center>', {
										OK: "OK"
									}, function(button) {
										if (button == 'OK') {} else {}
									});
							...
							error: function() {
								$.alert.open('Master kabupaten', '<center>Data gagal dihapus.</center>', {
									OK: "OK"
								}, function(button) {
									if (button == 'OK') {
										location.reload();
				...
				$.alert.open('Master kabupaten', '<center>Pilih baris yang akan dihapus.</center>', {
					OK: "OK",
				}, function(button) {
		...
		<div class="row">
			<div class="twelve columns">
				<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix">
					<h6>&nbsp;<img src="<?php echo base_url(); ?>images/crud/home.png">&nbsp;Master kabupaten</h6>
				</div>
				<hr>
							...
							<form id="form1" action="" method="post">
								<div class="row">
									...
									<div class="large-4 columns">
										<select name="select_pd_provinsi" id="select_pd_provinsi" tabindex="3"></select>
									</div>
								...
								<div class="row">
									<div class="large-3 columns">
										<label class="right inline"><span class="radius secondary label">Kode kabupaten <span style="color: red;font-size: 22px;">*</span> :</span></label>
									</div>
									<div class="large-2 columns">
										<input type="text" name="kode_kabupaten" id="kode_kabupaten" onBlur="javascript:{this.value = this.value.toUpperCase(); }" tabindex="1" />
									</div>
									<div class="large-1 columns">
									</div>
								</div>
								<div class="row">
									<div class="large-3 columns">
										<label class="right inline"><span class="radius secondary label">Nama kabupaten <span style="color: red;font-size: 22px;">*</span> :</span></label>
									</div>
									<div class="large-4 columns">
										<input type="text" name="nama_kabupaten" id="nama_kabupaten" tabindex="2" />
									</div>
									<div class="large-1 columns">
									</div>
								</div>
							...
							<form id="form2">
								<div class="row">
									<div class="large-12 columns">
										<div class="large-4 columns">
											<label class="right inline"><span class="radius secondary label">Search Kode kabupaten atau Nama : </label>
										</div>
										<div class="large-3 columns">
											<input type="text" id="search_kode_kabupaten" class='iblue' name="search_kode_kabupaten" tabindex="1" />
										</div>
										<hr>
